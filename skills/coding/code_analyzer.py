...
class CodeAnalyzer:
    ...
    def check_unused_imports(self, tree):
        ...
        class ImportChecker(ast.NodeVisitor):
            ...
            def visit_ImportFrom(self, node):
                for alias in node.names:
                    self.imports.add(
                        alias.name if alias.name != "*" else f"{node.module}.*"
                    )
                self.generic_visit(node)
    ...
    def calculate_python_metrics(code, tree):
        ...
        comment_pattern = r"^\s*#"
        lines = code.split("\n")
        metrics["comments"] = sum(
            1 for line in lines if re.match(comment_pattern, line)
        )

        # Calculate comment percentage
        if metrics["lines_of_code"] > 0:
            metrics["comment_percentage"] = (
                metrics["comments"] / metrics["lines_of_code"]
            ) * 100
        else:
    ...
    def explain_error(error_type, error_message):
        ...
        return explanations.get(
            error_type, "There's an issue with the code that needs fixing."
        )
