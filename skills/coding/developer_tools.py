...
class DeveloperTools:
    ...
    def npm_tools(action, args):
        ...
        try:
            ...
            if action not in commands:
                return {"success": False, "error": f"NPM action {action} not found"}
    ...
    def pip_tools(action, args):
        ...
        try:
            ...
            if action not in commands:
                return {"success": False, "error": f"PIP action {action} not found"}
            ...
            if action == "processes":
                ...
                if os.name == "nt":  # Windows
                    result = subprocess.run(
                        "tasklist", shell=True, capture_output=True, text=True
                    )
                else:  # Linux/Mac
                    result = subprocess.run(
                        "ps aux", shell=True, capture_output=True, text=True
                    )
                ...
                if os.name == "nt":
                    ...
                    result = subprocess.run(
                        "ifconfig", shell=True, capture_output=True, text=True
                    )
                ...
                return {"success": False, "error": f"System action {action} not found"}
    ...
    def web_tools(self, action, args):
        ...
        try:
            if action == "check_status":
                ...
                return {"success": True, "tool": "web", "action": action, "url": url}
                ...
                return {"success": False, "error": f"Web action {action} not found"}
    ...
    def api_tools(self, action, args):
        ...
        try:
            if action == "test_endpoint":
                ...
                if not url:
                    return {"success": False, "error": "URL required for API test"}
                ...
                return {"success": False, "error": f"API action {action} not found"}
    ...
    def run_tests(self, action, args):
        ...
        try:
            ...
            if not test_files:
                ...
                return {
                    "success": result.returncode == 0
                    or result.returncode == 5,  # 5 means no tests found
                    "tool": "test",
                    "action": "run_pytest",
                    "path": test_path,
                    "output": result.stdout,
    ...
    def build_project(self, action, args):
        ...
        try:
            ...
            if project_type == "python":
                ...
                if not os.path.exists("setup.py") and not os.path.exists(
                    "pyproject.toml"
                ):
...
"""
        ...
        try:
            ...
            if platform == "local":
                ...
                if os.path.exists("main.py"):
                    ...
                    result = subprocess.run(
                        "python app.py", shell=True, capture_output=True, text=True
                    )
                elif os.path.exists("index.js"):
                    result = subprocess.run(
                        "node index.js", shell=True, capture_output=True, text=True
                    )
                else:
                    return {"success": False, "error": "No main entry point found"}
    ...
    def monitor_system(self, action, args):
        ...
        try:
            ...
            self.tts.speak(
                f"System status: CPU {cpu_percent} percent, Memory {memory.percent} percent used"
            )
            ...
            return {"success": False, "error": "psutil library not installed"}
        except Exception as e:
            return {"success": False, "error": str(e)}
