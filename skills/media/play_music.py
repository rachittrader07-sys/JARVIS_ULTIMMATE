...
class MusicPlayer:
    ...
    def execute(self, params):
        ...
        if action in action_map:
            return action_map[action](
                playlist=playlist, song=song, genre=genre, params=params
            )
        else:
        ...
        try:
            if os.path.exists(self.playlists_file):
                ...
                default_playlists = {
                    ...
                    "settings": {"auto_save": True, "default_volume": 70},
                }
    ...
    def play_specific_song(self, song_query, params):
        ...
        try:
            ...
            if not found_songs:
                self.tts.speak(
                    f"Sir, I couldn't find {song_query} in your music library."
                )
                return {
                    "success": False,
            ...
            if len(found_songs) > 1:
                self.tts.speak(
                    f"Found {len(found_songs)} songs. Playing {os.path.basename(song_to_play)}"
                )
            ...
            result = media_controller.play_media(
                path=song_to_play, volume=params.get("volume", 70) if params else 70
            )
    ...
    def play_playlist(self, playlist="", song="", genre="", params=None):
        ...
        try:
            ...
            result = media_controller.play_media(
                path=first_song, volume=params.get("volume", 70) if params else 70
            )

            if result["success"]:
                self.tts.speak(
                    f"Playing playlist {playlist_data['name']}. {len(songs)} songs."
                )
    ...
    def play_random_music(self, params):
        ...
        try:
            ...
            if not music_library:
                ...
                return {"success": False, "error": "Music library empty"}
            ...
            result = media_controller.play_media(
                path=random_song, volume=params.get("volume", 70) if params else 70
            )
    ...
    def next_song(self, playlist="", song="", genre="", params=None):
        ...
        try:
            ...
            if not songs:
                ...
                return {"success": False, "error": "Playlist empty"}
    ...
    def create_playlist(self, playlist="", song="", genre="", params=None):
        ...
        try:
            if not playlist:
                ...
                return {"success": False, "error": "No playlist name provided"}
    ...
    def add_to_playlist(self, playlist="", song="", genre="", params=None):
        ...
        try:
            ...
            if not found_songs:
                ...
                return {"success": False, "error": "Song not found", "song": song}
            ...
            if self.save_playlists():
                self.tts.speak(
                    f"Added {os.path.basename(song_to_add)} to playlist {playlist}."
                )
                return {
                    "success": True,
    ...
    def list_playlists(self, playlist="", song="", genre="", params=None):
        ...
        try:
            ...
            if count == 1:
                ...
                self.tts.speak(
                    f"You have {count} playlists: {', '.join(playlist_names)}"
                )
    ...
    def list_songs(self, playlist="", song="", genre="", params=None):
        ...
        try:
            if playlist:
                ...
                self.tts.speak(
                    f"Playlist {playlist} has {len(songs)} songs. First few: {', '.join(song_names)}"
                )
    ...
    def current_song(self, playlist="", song="", genre="", params=None):
        ...
        try:
            ...
            self.tts.speak(
                f"Currently playing: {song_name}. Track {self.current_track_index + 1} of {len(songs)}."
            )
