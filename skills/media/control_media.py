...
class MediaController:
    ...
    def execute(self, params):
        ...
        if action in action_map:
            return action_map[action](
                path=media_path, url=media_url, volume=volume, params=params
            )
        else:
    ...
    def play_media(self, path="", url="", volume=50, params=None):
        ...
        try:
            ...
            if not media_source:
                self.tts.speak(
                    "Sir, what should I play? Please provide a file path or URL."
                )
            ...
            # Get player to use
            player_name = (
                params.get("player", self.default_player)
                if params
                else self.default_player
            )

            if not player_name or not self.media_players.get(player_name, {}).get(
                "available"
            ):
                self.tts.speak(
                    "Sir, no media player is available. Please install VLC or MPV."
                )
            ...
            if player_name == "vlc":
                ...
                cmd = [
                    player_path,
                    "--no-video" if params and params.get("audio_only") else "",
                    "--volume=" + str(volume),
                ]
    ...
    def resume_media(self, path="", url="", volume=50, params=None):
        ...
        try:
            if not self.current_media or self.is_playing:
                ...
                return {"success": False, "error": "No media to resume"}

            # Resume from the same source
            result = self.play_media(
                path=(
                    self.current_media["source"]
                    if not self.current_media["is_url"]
                    else ""
                ),
                url=(
                    self.current_media["source"] if self.current_media["is_url"] else ""
                ),
            ...
            if system_platform == "Windows":
                ...
                try:
                    ...
                    if os.path.exists(nircmd_path):
                        ...
                        self.tts.speak(
                            "Sir, I cannot control volume on Windows. Please install pycaw library."
                        )
                        return {
                            "success": False,
                ...
                try:
                    ...
                    subprocess.run(
                        ["osascript", "-e", volume_applescript],
                        check=True,
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.DEVNULL,
                    )
                ...
                self.tts.speak(
                    f"Sir, volume control not supported on {system_platform}"
                )
                return {
                    "success": False,
        ...
        try:
            ...
            if system_platform == "Windows":
                try:
                    ...
                    return {"success": True, "action": "mute", "platform": "windows"}
                except ImportError:
                    nircmd_path = os.path.join("tools", "nircmd.exe")
                    if os.path.exists(nircmd_path):
        ...
        try:
            ...
            if system_platform == "Windows":
                try:
                    ...
                    return {"success": True, "action": "unmute", "platform": "windows"}
                except ImportError:
                    nircmd_path = os.path.join("tools", "nircmd.exe")
                    if os.path.exists(nircmd_path):
        ...
        for search_dir in search_dirs:
            if os.path.exists(search_dir):
                for root, dirs, files in os.walk(search_dir):
                    for file in files:
                        if filename.lower() in file.lower() and any(
                            file.endswith(ext) for ext in media_extensions
                        ):
