...
class WebsiteResolver:
        ...
        try:
            if os.path.exists(self.websites_file):
                ...
                default_websites = {
                    ...
                    'categories': [
                        'search', 'entertainment', 'development', 'social', 
                        'shopping', 'reference', 'news', 'education', 'productivity'
                    ],
                    'settings': {
                        'auto_complete': True,
        ...
        try:
            ...
            if self.websites.get('settings', {}).get('auto_complete', True):
                ...
                if suggestions:
                    ...
                    self.tts.speak(f"Opening {site_data['name']} based on your request for {site_name}")
    ...
    def resolve_website(self, site_name='', custom_url='', params=None):
        ...
        try:
            ...
            if site_name_lower in websites:
                ...
                self.tts.speak(f"{site_name} is {site_data['name']} at {site_data['url']}")
            ...
            for site_key, site_data in websites.items():
                shortcuts = site_data.get('shortcuts', [])
                if site_name_lower in shortcuts:
                    self.tts.speak(f"{site_name} refers to {site_data['name']} at {site_data['url']}")
    ...
    def remove_website(self, site_name='', custom_url='', params=None):
        ...
        try:
            ...
            if self.save_websites():
                ...
                return {
                    'success': False,
                    'error': 'Failed to save after removal'
                }
    ...
    def list_websites(self, site_name='', custom_url='', params=None):
        ...
        try:
            ...
            if count == 1:
                self.tts.speak(f"You have 1 website configured: {website_list[0]['name']}")
            else:
                website_names = [w['name'] for w in website_list[:3]]
                self.tts.speak(f"You have {count} websites configured. First few: {', '.join(website_names)}")
    ...
    def create_shortcut(self, site_name='', custom_url='', params=None):
        ...
        try:
            ...
            if shortcut in shortcuts:
                self.tts.speak(f"Shortcut {shortcut} already exists for {site_data['name']}")
                return {
                    'success': False,
        ...
        try:
            ...
            if not category:
                ...
                if not category_websites:
                    ...
                    self.tts.speak(f"Found {len(category_websites)} websites in {category}: {', '.join(website_names)}")
