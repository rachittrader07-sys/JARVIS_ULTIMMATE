...
class URLOpener:
    ...
    def execute(self, params):
        ...
        if result["success"]:
            ...
            self.tts.speak(
                f"Sir, I couldn't find {website_name}. Would you like me to search for it?"
            )
    ...
    def open_website_multi_method(self, website_name):
        ...
        for pattern, url in common_patterns.items():
            if pattern in website_name:
                webbrowser.open(url)
                print(
                    Fore.GREEN + f"✅ Opened via pattern: {pattern}" + Style.RESET_ALL
                )
        ...
        if "." not in website_name:
            ...
            for url in possible_urls:
                if self.check_url_exists(url):
                    webbrowser.open(url)
                    print(
                        Fore.GREEN + f"✅ Opened via guessing: {url}" + Style.RESET_ALL
                    )
