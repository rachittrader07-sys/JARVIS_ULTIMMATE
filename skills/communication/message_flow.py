...
class MessageFlow:
    ...
    def add_message(self, conversation_id=None, message='', sender='user', receiver='jarvis', **kwargs):
    ...
    def get_conversation(self, conversation_id=None, **kwargs):
        ...
        try:
            ...
            self.tts.speak(f"Retrieved {len(paginated_messages)} messages from conversation.")
    ...
    def list_conversations(self, **kwargs):
        ...
        try:
            ...
            for conv_id, conversation in self.conversations.items():
                ...
                if filter_archived and conversation.get('settings', {}).get('archived', False):
                    continue
                ...
                conversations_list.append({
                    ...
                    'archived': conversation.get('settings', {}).get('archived', False),
    ...
    def search_messages(self, **kwargs):
        ...
        try:
            ...
            if conversation_id:
                if conversation_id in self.conversations:
                    conversations_to_search = [(conversation_id, self.conversations[conversation_id])]
                else:
                    self.tts.speak("Sir, that conversation doesn't exist.")
                    return {
                        'success': False,
                ...
                for message in conversation.get('messages', []):
                    ...
                    if query_lower in message['text'].lower():
                        results.append({
                            ...
                            'message': message,
                            'relevance': self.calculate_relevance(message['text'], query)
                        })
            ...
            if format_type == 'json':
                ...
                with open(filepath, 'w', encoding='utf-8') as f:
                    ...
                    for message in conversation.get('messages', []):
                        ...
                        f.write(f"[{timestamp}] {message['sender']}: {message['text']}\n")
    ...
    def get_summary(self, conversation_id=None, **kwargs):
        ...
        try:
            ...
            summary_text = f"Conversation '{conversation['title']}' has {len(messages)} messages "
            summary_text += f"({len(user_messages)} from you, {len(jarvis_messages)} from me). "
