...
class ContactResolver:
    ...
    def find_contact(self, name, phone, email, params):
        ...
        try:
            ...
            for contact in contacts_list:
                ...
                if (
                    search_term_lower in contact.get("name", "").lower()
                    or search_term_lower in contact.get("phone", "")
                    or search_term_lower in contact.get("email", "").lower()
                ):
            ...
            if found_contacts:
                if len(found_contacts) == 1:
                    ...
                    self.tts.speak(
                        f"I found {contact['name']}. Phone: {contact.get('phone', 'Not available')}. Email: {contact.get('email', 'Not available')}"
                    )
                    ...
                    self.tts.speak(
                        f"I found {len(found_contacts)} contacts: {', '.join(names)}"
                    )
                ...
                self.tts.speak(
                    f"Sorry sir, I couldn't find any contact matching {search_term}"
                )
                return {
                    "success": False,
                    "action": "find",
    ...
    def add_contact(self, name, phone, email, params):
        ...
        try:
            ...
            if phone and not self.validate_phone(phone):
                self.tts.speak(
                    "Sir, that phone number format is invalid. Please provide a valid number."
                )
            ...
            if email and not self.validate_email(email):
                self.tts.speak(
                    "Sir, that email format is invalid. Please provide a valid email."
                )
            ...
            if self.save_contacts():
                ...
                return {"success": True, "action": "add", "contact": new_contact}
            else:
    ...
    def update_contact(self, name, phone, email, params):
        ...
        try:
            ...
            if not contact_id and not search_name:
                ...
                return {"success": False, "error": "No contact identifier provided"}
            ...
            for i, contact in enumerate(contacts_list):
                if (contact_id and contact.get("id") == contact_id) or (
                    search_name
                    and search_name.lower() in contact.get("name", "").lower()
                ):
            ...
            if self.save_contacts():
                self.tts.speak(
                    f"Contact {contact_to_update['name']} updated successfully."
                )
                return {
                    "success": True,
                    "action": "update",
                ...
                return {"success": False, "error": "Failed to save updated contact"}
    ...
    def delete_contact(self, name, phone, email, params):
        ...
        try:
            ...
            if self.save_contacts():
                self.tts.speak(
                    f"Contact {contact_to_delete['name']} deleted successfully."
                )
                return {
                    "success": True,
                    "action": "delete",
            ...
            if name:
                ...
                if result["success"]:
                    ...
                    if contact and contact.get("phone"):
                        ...
                        self.tts.speak(
                            f"Sir, {name} doesn't have a phone number saved."
                        )
                        return {
                            "success": False,
            ...
            if name:
                ...
                if result["success"]:
                    ...
                    if contact and contact.get("email"):
                        ...
                        self.tts.speak(
                            f"Sir, {name} doesn't have an email address saved."
                        )
                        return {"success": False, "error": "No email for contact"}
                else:
                    self.tts.speak(f"Sir, I couldn't find {name} in your contacts.")
                    return result
        ...
        try:
            if not backup_path:
                backup_path = os.path.join(
                    "backups",
                    f'contacts_backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json',
                )
