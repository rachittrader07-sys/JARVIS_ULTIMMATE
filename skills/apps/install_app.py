...
class InstallApp:
    ...
    def execute(self, params):
        ...
        print(Fore.YELLOW + f"📦 Installing app: {app_name} from {source}" + Style.RESET_ALL)
            ...
            for i in range(2, len(lines)):
                if lines[i].strip():
                    ...
                    if len(parts) >= 2:
                        ...
                        # Install the app
                        install_cmd = ['winget', 'install', '--id', exact_name, '--silent', '--accept-package-agreements', '--accept-source-agreements']
                        install_result = subprocess.run(install_cmd, capture_output=True, text=True)
                        
                        if install_result.returncode == 0:
                            print(Fore.GREEN + f"  ✅ Installed via winget: {exact_name}" + Style.RESET_ALL)
                            return {'success': True, 'method': 'winget', 'app': exact_name}
                        else:
                            ...
                            if install_result.returncode == 0:
                                return {'success': True, 'method': 'winget_interactive', 'app': exact_name}
    ...
    def install_via_chocolatey(self, app_name):
        ...
        try:
            ...
            if install_result.returncode == 0:
                print(Fore.GREEN + f"  ✅ Installed via Chocolatey: {app_name}" + Style.RESET_ALL)
    ...
    def install_via_scoop(self, app_name):
        ...
        try:
            ...
            scoop_check = subprocess.run(['scoop', '--version'], capture_output=True, text=True)
            ...
            if install_result.returncode == 0:
                print(Fore.GREEN + f"  ✅ Installed via Scoop: {app_name}" + Style.RESET_ALL)
    ...
    def install_direct_download(self, app_name):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(Fore.GREEN + f"  ✅ Downloaded: {installer_path}" + Style.RESET_ALL)
                ...
                if install_result.returncode == 0:
                    print(Fore.GREEN + f"  ✅ Installed via direct download: {app_key}" + Style.RESET_ALL)
                    
                    # Cleanup installer
                    os.remove(installer_path)
                    
                    return {'success': True, 'method': 'direct_download', 'app': app_key}
                else:
                    # Try normal install
                    ...
                    if install_result.returncode == 0:
                        return {'success': True, 'method': 'direct_download_interactive', 'app': app_key}
                    else:
                        return {'success': False, 'error': 'Installation failed'}
            else:
