...
class CloseApp:
    ...
    def close_via_taskkill(self, app_name, force=False):
        ...
        try:
            ...
            cmd = ['taskkill', '/IM', f'*{app_name}*', '/F'] if force else ['taskkill', '/IM', f'*{app_name}*']
            
            # Execute
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
            
            if result.returncode == 0 or 'terminated' in result.stdout.lower():
                print(Fore.GREEN + f"✅ Closed via taskkill: {app_name}" + Style.RESET_ALL)
            ...
            for proc in psutil.process_iter(['pid', 'name']):
                try:
                    ...
                    if app_name in proc_name:
                        ...
                        processes_closed += 1
                        print(Fore.GREEN + f"✅ Terminated process: {proc_name}" + Style.RESET_ALL)
                        
                except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                    continue
            
            if processes_closed > 0:
                return {'success': True, 'method': 'process_termination', 'count': processes_closed}
            else:
                return {'success': False}
        ...
        try:
            ...
            for window in windows:
                try:
                    window.close()
                    windows_closed += 1
                    print(Fore.GREEN + f"✅ Closed window: {window.title}" + Style.RESET_ALL)
                except:
                    continue
            
            if windows_closed > 0:
                return {'success': True, 'method': 'window_close', 'count': windows_closed}
            else:
                return {'success': False}
    ...
    def close_current_app(self):
        ...
        try:
            ...
            if active_window:
                ...
                print(Fore.GREEN + f"✅ Closed current app: {app_name}" + Style.RESET_ALL)
    ...
    def force_close_unresponsive(self):
        ...
        try:
            ...
            if 'terminated' in result.stdout.lower() or result.returncode == 0:
                print(Fore.GREEN + "✅ Force closed unresponsive apps" + Style.RESET_ALL)
