...
class OpenApp:
    ...
    def execute(self, params):
        ...
        if result["success"]:
            ...
            self.tts.speak(
                f"Sir, I couldn't find {app_name}. Could you please specify the exact name?"
            )
    ...
    def open_via_start_menu(self, app_name):
        ...
        try:
            ...
            subprocess.run(
                ["powershell", "-Command", ps_command], shell=True, capture_output=True
            )
            print(
                Fore.GREEN
                + f"✅ Attempted to open {app_name} via Start Menu"
                + Style.RESET_ALL
            )
                                            ...
                                            for root, dirs, files in os.walk(
                                                ...
                                                for file in files:
                                                    if file.endswith(".exe"):
                                                        ...
                                                        print(
                                                            Fore.GREEN
                                                            + f"✅ Opened via registry: {display_name}"
                                                            + Style.RESET_ALL
                                                        )
                                                        return {
                                                            "success": True,
    ...
    def try_appopener(self, app_name):
        ...
        try:
            ...
            print(
                Fore.GREEN + f"✅ Opened using appopener: {app_name}" + Style.RESET_ALL
            )
    ...
    def try_system_search(self, app_name):
        ...
        try:
            ...
            print(
                Fore.GREEN
                + f"✅ Attempted Windows search for: {app_name}"
                + Style.RESET_ALL
            )
