...
class ToneManager:
    ...
    def initialize_engine(self):
        ...
        try:
            ...
            for voice in voices:
                if 'indian' in voice.name.lower() or 'hindi' in voice.name.lower():
                    if 'male' in voice.name.lower():
                        ...
                        print(Fore.GREEN + f"✅ Indian male voice found: {voice.name}" + Style.RESET_ALL)
                        break
            ...
            print(Fore.RED + f"❌ Tone manager initialization failed: {str(e)}" + Style.RESET_ALL)
    ...
    def set_tone(self, tone_name):
        ...
        if self.engine:
            ...
            print(Fore.CYAN + f"🎵 Tone set to: {tone_name} (rate: {settings['rate']}, volume: {settings['volume']})" + Style.RESET_ALL)
    ...
    def adjust_tone_dynamically(self, emotion, intensity=0.5):
        ...
        if emotion in ["happy", "excited"]:
            adjusted_settings['volume'] = min(1.0, base_settings['volume'] * (0.8 + intensity))
        elif emotion in ["sad", "calm"]:
            adjusted_settings['volume'] = max(0.3, base_settings['volume'] * (0.5 + intensity))
    ...
    def speak_with_tone(self, text, tone=None):
        ...
        try:
            ...
            # Speak
            print(Fore.CYAN + f"🗣️ Speaking with {self.current_tone} tone: {text[:50]}..." + Style.RESET_ALL)
    ...
    def save_tone_preferences(self):
        ...
        try:
            ...
            print(Fore.YELLOW + f"⚠️ Tone preferences save error: {str(e)}" + Style.RESET_ALL)
            return False
    ...
    def load_tone_preferences(self):
        ...
        try:
            with open('memory/tone_preferences.json', 'r', encoding='utf-8') as f:
                ...
                print(Fore.GREEN + f"✅ Tone preferences loaded: {tone}" + Style.RESET_ALL)
            ...
            print(Fore.YELLOW + f"⚠️ Tone preferences load error: {str(e)}" + Style.RESET_ALL)
            return False
