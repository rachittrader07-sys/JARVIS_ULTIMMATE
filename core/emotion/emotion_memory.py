...
class EmotionMemory:
    ...
    def record_interaction(
        self, user_input, jarvis_response, user_emotion, jarvis_emotion
    ):
    ...
    def find_similar_pattern(self, pattern):
        ...
        for p in self.emotional_patterns:
            if (
                p["type"] == pattern["type"]
                and p["user_input"] == pattern["user_input"]
            ):
                return p
        return None
    ...
    def get_user_mood_insights(self):
        ...
        if positivity_scores:
            insights["best_time_of_day"] = max(
                positivity_scores, key=positivity_scores.get
            )
            insights["worst_time_of_day"] = min(
                positivity_scores, key=positivity_scores.get
            )
    ...
    def save_memory(self):
        ...
        try:
            ...
            print(
                Fore.YELLOW + f"⚠️ Emotion memory save error: {str(e)}" + Style.RESET_ALL
            )
    ...
    def load_memory(self):
        ...
        try:
            if os.path.exists("memory/emotion_memory.json"):
                ...
                print(
                    Fore.GREEN
                    + f"❤️ Emotion memory loaded: {len(self.interaction_history)} interactions"
                    + Style.RESET_ALL
                )

        except Exception as e:
            print(
                Fore.YELLOW + f"⚠️ Emotion memory load error: {str(e)}" + Style.RESET_ALL
            )
