...
class ContextTracker:
    ...
    def update_context(
        self, user_input, assistant_response, intent=None, entities=None
    ):
        ...
        try:
            ...
            print(
                Fore.CYAN
                + f"🗣️ Context updated: {context_entry['topic']}"
                + Style.RESET_ALL
            )
    ...
    def detect_topic(self, user_input):
        ...
        if any(word in input_lower for word in ["youtube", "video", "watch"]):
            ...
            word in input_lower for word in ["open", "close", "app", "application"]
        ):
            return "apps"
        elif any(
            word in input_lower for word in ["code", "program", "python", "write"]
        ):
            return "coding"
    ...
    def get_context_for_intent(self, intent):
        ...
        if intent in ["send_whatsapp", "whatsapp_message"]:
            ...
            relevant_context["last_message"] = self.get_last_interaction_with(
                "whatsapp"
            )
    ...
    def load_context(self):
        ...
        try:
            with open("memory/context_data.json", "r", encoding="utf-8") as f:
                ...
                print(
                    Fore.GREEN
                    + f"🗣️ Context loaded: {len(self.topic_stack)} topics, {len(self.entity_context)} entity types"
                    + Style.RESET_ALL
                )
