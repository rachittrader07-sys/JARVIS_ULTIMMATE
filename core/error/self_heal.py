...
class SelfHeal:
    ...
    def diagnose_and_fix(self, error_type, error_details, context):
        ...
        print(
            Fore.YELLOW + f"🛠️ Attempting self-heal for: {error_type}" + Style.RESET_ALL
        )
        ...
        for fix in fixes:
            ...
            if success:
                print(
                    Fore.GREEN
                    + f"  ✅ Fix successful: {fix['description']}"
                    + Style.RESET_ALL
                )
        ...
        return {"healed": False, "fix_applied": None, "all_results": results}
    ...
    def get_suggested_fixes(self, error_type, error_details, context):
        ...
        error_patterns = {
            ...
            "import_error": [
                ...
                {"description": "Update pip", "action": "update_pip", "priority": 2},
                {
    ...
    def restart_audio_services():
        ...
        try:
            if sys.platform == "win32":
                ...
                subprocess.run(
                    ["net", "stop", "Audiosrv"], shell=True, capture_output=True
                )
                time.sleep(2)
                subprocess.run(
                    ["net", "start", "Audiosrv"], shell=True, capture_output=True
                )
                time.sleep(2)
                return True
            return False
    ...
    def run_system_health_check(self):
        ...
        for check_name, check_func in checks:
            ...
            success = check_func()
            results.append(
                {"check": check_name, "status": "✅ PASS" if success else "❌ FAIL"}
            )
