...
class ErrorMemory:
    ...
    def record_error(self, error_type, error_message, context, solution=None, success=False):
        """📊 Record an error"""
        error_entry = {
        ...
        print(Fore.YELLOW + f"📊 Error recorded: {error_type} in {context}" + Style.RESET_ALL)
    ...
    def find_similar_error(self, error_type, error_message, context):
        ...
        for error in self.error_history:
            ...
            if error['error_type'] == error_type:
                ...
                msg_similarity = self.calculate_similarity(error['error_message'], error_message)
    ...
    def update_error_pattern(self, error_entry):
        ...
        for pattern in self.error_patterns:
            if pattern['error_type'] == error_entry['error_type']:
                ...
                # Update success rate
                successful_solutions = sum(1 for sol in pattern['solutions_tried'] if sol['success'])
    ...
    def get_suggested_solution(self, error_type, context):
        ...
        for pattern in self.error_patterns:
            if pattern['error_type'] == error_type:
                ...
                successful_solutions = [sol for sol in pattern['solutions_tried'] if sol['success']]
    ...
    def get_error_stats(self, days=30):
        ...
        if stats['error_types']:
            stats['most_common_error'] = max(stats['error_types'].items(), key=lambda x: x[1])[0]
    ...
    def cleanup_old_errors(self, days=90):
        ...
        # Filter recent errors
        self.error_history = [e for e in self.error_history if e['timestamp'] >= cutoff_str]
        
        removed = old_count - len(self.error_history)
        
        # Also cleanup patterns with no recent occurrences
        self.error_patterns = [p for p in self.error_patterns if p['last_seen'] >= cutoff_str]
    ...
    def save_memory(self):
        ...
        try:
            ...
            print(Fore.GREEN + f"📊 Error memory saved: {len(self.error_history)} errors" + Style.RESET_ALL)
            
        except Exception as e:
            print(Fore.YELLOW + f"⚠️ Error memory save error: {str(e)}" + Style.RESET_ALL)
    
    def load_memory(self):
        """📊 Load error memory from file"""
        try:
            if os.path.exists(self.config['paths']['error_log']):
                with open(self.config['paths']['error_log'], 'r', encoding='utf-8') as f:
                    memory_data = json.load(f)
                    
                    self.error_history = memory_data.get('error_history', [])
                    self.error_patterns = memory_data.get('error_patterns', [])
                    self.solutions_database = memory_data.get('solutions_database', self.solutions_database)
                    
                print(Fore.GREEN + f"📊 Error memory loaded: {len(self.error_history)} errors" + Style.RESET_ALL)
                
        except Exception as e:
            print(Fore.YELLOW + f"⚠️ Error memory load error: {str(e)}" + Style.RESET_ALL)
        ...
        for pattern in self.error_patterns:
            if pattern['occurrence_count'] > 3:
                insight = {
                    ...
                    'common_context': pattern['common_contexts'][0] if pattern['common_contexts'] else 'unknown',
                    'best_solution': None,
    ...
    def export_errors_report(self, filepath="error_report.json"):
        ...
        try:
            ...
            print(Fore.GREEN + f"📊 Error report exported: {filepath}" + Style.RESET_ALL)
            return True
            
        except Exception as e:
            print(Fore.RED + f"❌ Error report export failed: {str(e)}" + Style.RESET_ALL)
            return False