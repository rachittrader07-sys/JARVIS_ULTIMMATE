...
class STTEngine:
    ...
    def listen(self, timeout=5, phrase_time_limit=8):
        ...
        try:
            with self.microphone as source:
                print(Fore.GREEN + self.config['indicators']['listening'] + " Listening... (Speak now)" + Style.RESET_ALL)
                ...
                print(Fore.YELLOW + self.config['indicators']['thinking'] + " Processing speech..." + Style.RESET_ALL)
        ...
        for method_name, method_func in methods:
            try:
                ...
                if text and len(text.strip()) > 1:
                    print(Fore.GREEN + f"✅ Recognized with {method_name}: {text}" + Style.RESET_ALL)
                    return text
            except Exception as e:
                print(Fore.YELLOW + f"⚠️ {method_name} failed: {str(e)}" + Style.RESET_ALL)
                continue
