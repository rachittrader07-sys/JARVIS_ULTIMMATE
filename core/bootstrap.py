...
class SystemBootstrap:
    ...
    def check_system_resources(self):
        ...
        try:
            ...
            if cpu_percent < 80:
                print(
                    Fore.GREEN + f"✅ CPU Usage: {cpu_percent}% - OK" + Style.RESET_ALL
                )
            else:
                self.warnings.append(f"High CPU usage: {cpu_percent}%")
                print(
                    Fore.YELLOW
                    + f"⚠️ CPU Usage: {cpu_percent}% - High"
                    + Style.RESET_ALL
                )
            ...
            if ram.percent < 85:
                print(
                    Fore.GREEN + f"✅ RAM Usage: {ram.percent}% - OK" + Style.RESET_ALL
                )
            else:
                self.warnings.append(f"High RAM usage: {ram.percent}%")
                print(
                    Fore.YELLOW
                    + f"⚠️ RAM Usage: {ram.percent}% - High"
                    + Style.RESET_ALL
                )
            ...
            if disk.percent < 90:
                print(
                    Fore.GREEN
                    + f"✅ Disk Space: {disk.percent}% used - OK"
                    + Style.RESET_ALL
                )
            else:
                ...
                print(
                    Fore.YELLOW
                    + f"⚠️ Disk Space: {disk.percent}% used - Low"
                    + Style.RESET_ALL
                )
    ...
    def print_summary(self):
        ...
        if not self.issues and not self.warnings:
            print(
                Fore.GREEN + "✅ All checks passed! System is ready." + Style.RESET_ALL
            )
        else:
            if self.issues:
