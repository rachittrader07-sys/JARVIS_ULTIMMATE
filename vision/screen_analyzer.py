...
class ScreenAnalyzer:
    ...
    def find_error_popups(self):
        ...
        try:
            ...
            for lower, upper in error_colors:
                ...
                for contour in contours:
                    if cv2.contourArea(contour) > 1000:  # Minimum size
                        ...
                        if text and any(word in text.lower() for word in ['error', 'failed', 'invalid', 'problem', 'issue']):
                            errors.append({
                                'x': x,
                                'y': y,
                                'width': w,
            ...
            print(Fore.RED + f"❌ Error popup detection failed: {str(e)}" + Style.RESET_ALL)
            return []
    ...
    def analyze_current_window(self):
        ...
        try:
            ...
            x, y, width, height = windows.left, windows.top, windows.width, windows.height
    ...
    def find_errors_in_image(self, image):
        ...
        try:
            ...
            error_keywords = [
                'error', 'failed', 'invalid', 'problem', 'issue',
                'cannot', 'unable', 'not found', 'missing', 'corrupt',
                'access denied', 'permission denied', 'timeout', 'crash'
            ]
