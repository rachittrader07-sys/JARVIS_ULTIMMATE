...
"""
    ...
    def check_python_version(self):
        ...
        print(
            f"{Fore.GREEN}✅ Python {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro} detected{Style.RESET_ALL}"
        )
        return True
    ...
    def create_directories(self):
        ...
        for directory in directories:
            try:
                ...
                print(
                    f"  {Fore.RED}✗{Style.RESET_ALL} Failed to create {directory}: {e}"
                )
    ...
    def copy_config_files(self):
        ...
        if os.path.exists(self.config_file) and not os.path.exists(self.config_target):
            try:
                ...
                print(
                    f"  {Fore.GREEN}✓{Style.RESET_ALL} Created config.yaml from default"
                )
            except Exception as e:
                print(f"  {Fore.RED}✗{Style.RESET_ALL} Failed to copy config: {e}")
        elif os.path.exists(self.config_target):
            print(
                f"  {Fore.YELLOW}⚠{Style.RESET_ALL} config.yaml already exists, keeping existing"
            )
        else:
            print(
                f"  {Fore.RED}✗{Style.RESET_ALL} Default config not found: {self.config_file}"
            )
        ...
        if not os.path.exists(commands_file):
            ...
            try:
                ...
                print(
                    f"  {Fore.RED}✗{Style.RESET_ALL} Failed to create commands.json: {e}"
                )
    ...
    def install_python_dependencies(self):
        ...
        if not os.path.exists(self.requirements_file):
            ...
            try:
                ...
                print(
                    f"  {Fore.RED}✗{Style.RESET_ALL} Failed to create requirements.txt: {e}"
                )
                return False
        ...
        try:
            ...
            if result.returncode == 0:
                print(
                    f"  {Fore.GREEN}✓{Style.RESET_ALL} Dependencies installed successfully"
                )
                return True
            else:
    ...
    def install_system_dependencies(self):
        ...
        if self.system == "Windows":
            ...
            print(
                f"  {Fore.YELLOW}⚠{Style.RESET_ALL} Unsupported system: {self.system}"
            )
            return True
    ...
    def install_windows_dependencies(self):
        ...
        try:
            ...
            if has_choco:
                packages = ["ffmpeg", "vlc", "python3", "git"]
                ...
                for package in packages:
                    try:
                        ...
                        print(
                            f"    {Fore.YELLOW}⚠{Style.RESET_ALL} Failed to install {package}"
                        )
            ...
            print(
                f"  {Fore.YELLOW}⚠{Style.RESET_ALL} Windows dependency installation had issues: {e}"
            )
    ...
    def install_linux_dependencies(self):
        ...
        try:
            ...
            if os.path.exists("/usr/bin/apt-get"):
                ...
                subprocess.run(
                    ["sudo", "apt-get", "install", "-y"] + packages, check=True
                )
                ...
                subprocess.run(
                    ["sudo", "pacman", "-S", "--noconfirm"] + packages, check=True
                )
            ...
            print(
                f"  {Fore.YELLOW}⚠{Style.RESET_ALL} Linux dependency installation had issues: {e}"
            )
            return True  # Continue anyway
    ...
    def install_macos_dependencies(self):
        ...
        try:
            ...
            if has_brew:
                packages = ["portaudio", "ffmpeg", "vlc", "espeak", "git"]
            ...
            print(
                f"  {Fore.YELLOW}⚠{Style.RESET_ALL} macOS dependency installation had issues: {e}"
            )
            return True  # Continue anyway
        ...
        try:
            ...
            for tool in tools:
                ...
                if not os.path.exists(tool_path):
                    ...
                    print(
                        f"    {Fore.GREEN}✓{Style.RESET_ALL} {tool['name']} (placeholder)"
                    )
    ...
    def setup_voice_profile(self):
        ...
        try:
            ...
            print(
                f"  {Fore.YELLOW}⚠{Style.RESET_ALL} Failed to create voice settings: {e}"
            )
        ...
        for skill_dir in skill_dirs:
            ...
            if os.path.exists(skill_dir) and not os.path.exists(init_file):
                try:
                    ...
                    print(
                        f"  {Fore.YELLOW}⚠{Style.RESET_ALL} Failed to create {init_file}: {e}"
                    )
        ...
        for skill_path, skill_content in sample_skills.items():
            if not os.path.exists(skill_path):
                try:
                    ...
                    print(
                        f"  {Fore.GREEN}✓{Style.RESET_ALL} Created sample skill: {skill_path}"
                    )
                except Exception as e:
                    print(
                        f"  {Fore.YELLOW}⚠{Style.RESET_ALL} Failed to create {skill_path}: {e}"
                    )
...
"""
            try:
                ...
                print(
                    f"{Fore.YELLOW}⚠ Failed to create start_j.bat: {e}{Style.RESET_ALL}"
                )
